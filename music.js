...
export class MusicPlayer {
    ...
    init() {
        ...
        if (this.dom.wnList) {
            this.dom.wnList.onclick = e => {
                ...
                if (item) {
                    this.playWnTrack(parseInt(item.dataset.index));
                }
            };
        }
    ...
    updateVolume(v) {
        this.mediaVol = v;
        if (this.dom.vol) {
            this.dom.vol.value = v;
            this.dom.vol.style.setProperty('--percent', v + '%');
        }
        if (this.dom.wnVol) {
            this.dom.wnVol.value = v;
            this.dom.wnVol.style.setProperty('--percent', v + '%');
        }
    ...
    switchTab(tab) {
        const tYt = document.getElementById('tab-btn-yt');
        const tWn = document.getElementById('tab-btn-wn');
        const cYt = document.getElementById('music-tab-yt');
        const cWn = document.getElementById('music-tab-wn');
        if (tab === 'yt') {
            tYt.classList.replace('opacity-50', 'opacity-100');
            tYt.classList.replace('border-transparent', 'border-black');
            tYt.classList.add('dark:border-white');
            tWn.classList.replace('opacity-100', 'opacity-50');
            tWn.classList.replace('border-black', 'border-transparent');
            tWn.classList.remove('dark:border-white');
            cYt.classList.remove('hidden');
            cYt.classList.add('flex');
            cWn.classList.add('hidden');
            cWn.classList.remove('flex');
        } else {
            tWn.classList.replace('opacity-50', 'opacity-100');
            tWn.classList.replace('border-transparent', 'border-black');
            tWn.classList.add('dark:border-white');
            tYt.classList.replace('opacity-100', 'opacity-50');
            tYt.classList.replace('border-black', 'border-transparent');
            tYt.classList.remove('dark:border-white');
            cWn.classList.remove('hidden');
            cWn.classList.add('flex');
            cYt.classList.add('hidden');
            cYt.classList.remove('flex');
        }
    }
    initYT() {
                ...
                'onError': e => {
                    if (this.currIdx >= 0 && this.currIdx < this.queue.length) {
                        ...
                        if (this.queue.length > 0) {
                            ...
                            setTimeout(() => { this.syncAndPlay(); }, 800);
                        } else {
        ...
        this.wnPlayer = new YT.Player('wn-player-container', {
            ...
            events: {
                ...
                'onError': () => {
                    if (this.dom.wnStat) this.dom.wnStat.innerText = "Error loading sound";
                    if (this.dom.wnIPlay) this.dom.wnIPlay.classList.remove('hidden');
                    if (this.dom.wnIPause) this.dom.wnIPause.classList.add('hidden');
                }
            }
        });
    }
        ...
        while (this.titleQueue.length > 0) {
            ...
            if (validBatch.length > 0) {
                await Promise.all(validBatch.map(async (q) => {
                    ...
                    if (!ok) { q.title = "Cannot load music title!"; }
                }));
                this.render();
            }
    ...
    onState(e) {
        ...
        if (e.data === YT.PlayerState.PLAYING) {
            if (this.wnPlayer && this.wnPlayer.getPlayerState && this.wnPlayer.getPlayerState() === YT.PlayerState.PLAYING) {
                this.wnPlayer.pauseVideo();
            }
    ...
    onWnState(e) {
        if (e.data === YT.PlayerState.PLAYING) {
            if (this.ytPlayer && this.ytPlayer.getPlayerState && this.ytPlayer.getPlayerState() === YT.PlayerState.PLAYING) {
                this.ytPlayer.pauseVideo();
            }
            if (this.dom.wnIPlay) this.dom.wnIPlay.classList.add('hidden');
            if (this.dom.wnIPause) this.dom.wnIPause.classList.remove('hidden');
        } else if (e.data === YT.PlayerState.PAUSED) {
            if (this.dom.wnIPlay) this.dom.wnIPlay.classList.remove('hidden');
            if (this.dom.wnIPause) this.dom.wnIPause.classList.add('hidden');
        }
    ...
    renderWnList() {
        ...
        this.wnTracks.forEach((track, i) => {
            ...
            if (i === this.activeWnIdx) {
                li.classList.replace('border-black/10', 'border-black');
                li.classList.replace('dark:border-white/10', 'dark:border-white');
                li.classList.add('bg-gray-100', 'dark:bg-gray-800');
                c.querySelector('.wn-title').classList.remove('opacity-70');
                c.querySelector('.wn-title').classList.add('font-bold');
            }
            c.querySelector('.wn-icon').innerHTML = track.icon;
            c.querySelector('.wn-title').textContent = track.title;            
            frag.appendChild(c);
        });
