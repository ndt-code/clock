...
export class StopwatchComponent {
    ...
    start() { this.startTime = Date.now(); this.isRunning = true; this.runAnimationLoop(); this.toggleBtns(true) }
    ...
    lap() {
        ...
        const t = UIUtils.parseMs(total);
        const cur = `${t.h.toString().padStart(2, '0')}:${t.m.toString().padStart(2, '0')}:${t.s.toString().padStart(2, '0')}.${t.ms.toString().padStart(2, '0')}`;
        const tmpl = document.getElementById('tpl-lap-item'), c = tmpl.content.cloneNode(true); 
        c.querySelector('.lap-num').textContent = `Lap ${this.dom.lapListD ? this.dom.lapListD.children.length + 1 : 1}`; 
        c.querySelector('.lap-time').textContent = cur;
        const c2 = c.cloneNode(true); 
        if(this.dom.lapListD) this.dom.lapListD.prepend(c); 
        if(this.dom.lapListM) this.dom.lapListM.prepend(c2);
    }
    toggleBtns(run) {
